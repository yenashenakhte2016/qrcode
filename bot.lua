...
local help = [[_BarCodRobot_ *(Inline)*
 
*Inline Mode*
     `enter this robot username (@BarcodRobot) in the Input Text Box and press Space button and wait for show Inline Keyboard, if you have Username, in Inline Keyboard show a button for make QR Code for you'r username. You can enter text after this robot username for make QR Core of input text.`
...
you have 2 algorithm for make Hash Code:
     ...
     _About:_
     `you can send utf-8 texts for encode hash to algorithm About`
...
api.sendPhoto(msg.chat.id,'./images/bc.png','@barcodrobot',msg.message_id)
api.sendMessage(msg.chat.id, '*Main Menu:*', true, true,nil, true,make_menu())
db:hset('bot:waiting',msg.from.id,'main')
else
api.sendMessage(msg.chat.id, 'Input is *False*', true, true,msg.message_id, true)
...
local rw1_texts = {'🔲🔳\n🔳🔲'}
local rows ={kmakerow(rw1_texts)}
api.sendMessage(msg.chat.id, 'Send target *Username*\n`example: @MohammadDeveloper`', true, true,msg.message_id, true,kmake(rows))
...
api.sendPhoto(msg.chat.id,api.downloadFile('http://api.qrserver.com/v1/create-qr-code/?data=https://telegram.me/'..msg.from.username..'&color=0FA0EF&size=512x512','./images/username.png'),'@barcodrobot',msg.message_id)
api.sendMessage(msg.chat.id, '*Main Menu:*', true, true,nil, true,make_menu())
db:hset('bot:waiting',msg.from.id,'main')
...
api.sendPhoto(msg.chat.id,api.downloadFile('http://api.qrserver.com/v1/create-qr-code/?data=https://telegram.me/'..msg.text:gsub('^@','')..'&color=0FA0EF&size=512x512','./images/username.png'),'@barcodrobot',msg.message_id)
api.sendMessage(msg.chat.id, '*Main Menu:*', true, true,nil, true,make_menu())
db:hset('bot:waiting',msg.from.id,'main')
else
api.sendMessage(msg.chat.id, 'Input is *False*', true, true,msg.message_id, true)
...
local sdata = json:decode(db:hget('bot:screate',msg.from.id))
api.sendPhoto(msg.chat.id,api.downloadFile('http://api.qrserver.com/v1/create-qr-code/?margin='..(tonumber(sdata.boarder) * 10)..'&color='..sdata.color..'&bgcolor='..sdata.bgcolor..'&size='..sdata.size..'&data='..msg.text,'./images/image.png'),'@barcodrobot',msg.message_id)
api.sendMessage(msg.chat.id, '*Main Menu:*', true, true,nil, true,make_menu())
db:hset('bot:waiting',msg.from.id,'main')
...
qresult[1].message_text = 'You haven\'t *Username* for make telegram QR!\n`you can enter text after this bot username (@barcodrobot) and make QR code of you\'r text`'
...
qresult[1].id = '1'
 ...
 qresult[1].caption = URL.escape('@barcodrobot')
 api.sendInline(inline.id, qresult,0)
end
end
